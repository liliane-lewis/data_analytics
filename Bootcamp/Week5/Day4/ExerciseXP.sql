...
SELECT * from customers ORDR BY firstname LIMIT TO 3;

-- All last names (no other columns!), in reverse alphabetical order (Z-A)
SELECT lastname from customers DESC;
...
--Lets get all the dates that accounts were created. Write a query to select all the create_date from the 
-- “customer” table (there should be no duplicates).
SELECT DISTINCT create_date FROM customer;

-- Write a query to get all the customer details from the customer table, it should be displayed in descending order by their first name.
SELECT * FROM customer ORDER BY first_name;

-- Write a query to get the film ID, title, description, year of release and rental rate in ascending order according to their 
-- rental rate.
SELECT film_id, title, description, release_year, rental_rate FROM film ORDER BY rental_rate ASC;

--Write a query to get the address, and the phone number of all customers living in the Texas district, these details can 
-- be found in the “address” table.
...
SELECT * FROM ( SELECT *, ROW_NUMBER() OVER (ORDER BY rental_rate ASC) AS rn FROM film ) sub WHERE rn > 10 AND rn <= 20;
